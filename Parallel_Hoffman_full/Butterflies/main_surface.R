...
write.csv(pd_model, "butterflies/GENUSpd_model_params.csv", row.names = TRUE)
write.csv(mpd_model, "butterflies/GENUSmpd_model_params.csv", row.names = TRUE)
write.csv(mntd_model, "butterflies/GENUSmntd_model_params.csv", row.names = TRUE)

json_pd<- toJSON(x = pd_model, dataframe = 'rows', pretty = F)
write(json_pd, file = "butterflies/GENUSpd_model_params.json")

json_mpd<- toJSON(x = mpd_model, dataframe = 'rows', pretty = F)
write(json_mpd, file = "butterflies/GENUSmpd_model_params.json")

json_mntd<- toJSON(x = mntd_model, dataframe = 'rows', pretty = F)
write(json_mntd, file = "butterflies/GENUSmntd_model_params.json")
...
for(i in dir_list_ecoregions)
{
  ...
  pd_data_temp_mam<- read.csv(paste(i, "/ecoregions_GENUSCI_pd_output_bootstrap_bird.csv", sep = ""))
  ...
  write(json_pd, file = paste(i, "/GENUSpd_model_params.json", sep = ""))
  
  json_mpd<- toJSON(x = mpd_model_temp_mam, dataframe = 'rows', pretty = F)
  write(json_mpd, file = paste(i,"/GENUSmpd_model_params.json", sep = ""))
  
  json_mntd<- toJSON(x =mntd_model_temp_mam, dataframe = 'rows', pretty = F)
  write(json_mntd, file = paste(i, "/GENUSmntd_model_params.json", sep = ""))
