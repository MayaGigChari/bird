#population hexagons for squamate
...
cophen_squamate<-readRDS("squamate/cali_cophen_matrix")
#parent_tree<- read.tree("Plants/interpolated_full_tree.tre") probably don't really need this.
#made the genus tree in main. 
cali_tree_squamate<- read.tree("squamate/cali_tree.tre") #load the genus tree
cali_species_ranges<- st_read("squamate/squamate_complete_species_lists_by_ecoregion.shp")
...
st_write(hex_species_ranges, "squamate/full_hex_intersect_ranges_joined.shp", append = FALSE)
...
squamate_hex_species_list<- genHexRaw_prejoined(hex_species_ranges, h3_indexes)

names(squamate_hex_species_list)<- h3_indexes



poly_labels<- names(squamate_hex_species_list) #this gives us all the codes. 
...
hex_tree_stats_squamate<- popHexStats(squamate_hex_species_list, cali_tree_squamate, "squamate", cophen_filepath = "squamate/cali_cophen_matrix")

hex_tree_stats_plants_copy_squamate<- hex_tree_stats_squamate

pop_hex_stats_squamate_df<- data_frame("h3_index" = names(squamate_hex_species_list), "pd_values" = unlist(hex_tree_stats_squamate$pd_values), "mpd_values" = unlist(hex_tree_stats_squamate$mpd_values), "mntd_values" = unlist(hex_tree_stats_squamate$mntd_values), 
                                      "tree_size" = unlist(lapply(hex_tree_stats_squamate$species_in_tree, length)), "missing_taxa_tree_size" = unlist(lapply(hex_tree_stats_squamate$missing_species, length)) )

#seems that there are generally about 40% of taxa missing. this might fuck everything up but who knows. 
pop_hex_stats_squamate_df$proportion_missing<- pop_hex_stats_squamate_df$missing_taxa_tree_size/(pop_hex_stats_squamate_df$tree_size + pop_hex_stats_squamate_df$missing_taxa_tree_size)


histogram(pop_hex_stats_squamate_df$proportion_missing)
#this is just an aside to make a figure of mssing taxa. 

#need to make 

library(ggplot2)
missing_hist<- ggplot(pop_hex_stats_squamate_df, aes(x = proportion_missing)) +
  geom_histogram(bins = 50) +
  ggtitle("Missing Taxa Proportions: Hexagon Data California")

ggsave(filename = "squamate/Missing_taxa_proportions_hexagon_california_species.png", 
       plot = missing_hist, 
       dpi = 300, 
       width = 8, 
       height = 6, 
...
polygon_data_full_squamate_with_eco$ec_js_pd<- unlist(lapply(polygon_data_full_squamate_with_eco$US_L3CODE, makejsonstring, clade = "squamate",  metric = "pd"))
polygon_data_full_squamate_with_eco$ec_js_mpd<- unlist(lapply(polygon_data_full_squamate_with_eco$US_L3CODE, makejsonstring, clade = "squamate", metric = "mpd"))
polygon_data_full_squamate_with_eco$ec_js_mntd<- unlist(lapply(polygon_data_full_squamate_with_eco$US_L3CODE, makejsonstring,clade = "squamate", metric = "mntd"))
...
CI_cali_significance_polygons_pd_squamate[sapply(CI_cali_significance_polygons_pd_squamate, is.null)] <- NA
CI_cali_significance_polygons_mpd_squamate[sapply(CI_cali_significance_polygons_mpd_squamate, is.null)] <- NA
CI_cali_significance_polygons_mntd_squamate[sapply(CI_cali_significance_polygons_mntd_squamate, is.null)] <- NA

CI_ecoregions_significance_polygons_pd_squamate[sapply(CI_ecoregions_significance_polygons_pd_squamate, is.null)] <- NA
CI_ecoregions_significance_polygons_mpd_squamate[sapply(CI_ecoregions_significance_polygons_mpd_squamate, is.null)] <- NA
CI_ecoregions_significance_polygons_mntd_squamate[sapply(CI_ecoregions_significance_polygons_mntd_squamate, is.null)] <- NA

polygon_data_full_squamate_with_eco$pdSigCal<- unlist(CI_cali_significance_polygons_pd_squamate)
polygon_data_full_squamate_with_eco$mpdSigCal<- unlist(CI_cali_significance_polygons_mpd_squamate)
polygon_data_full_squamate_with_eco$mntdSigCal<- unlist(CI_cali_significance_polygons_mntd_squamate)


#need to change NULL to NA to make this datastructure work. 
#make NA's. line = 

polygon_data_full_squamate_with_eco$pdSigEco<- unlist(CI_ecoregions_significance_polygons_pd_squamate)
polygon_data_full_squamate_with_eco$mpdSigEco<- unlist(CI_ecoregions_significance_polygons_mpd_squamate)
polygon_data_full_squamate_with_eco$mntdSigEco<- unlist(CI_ecoregions_significance_polygons_mntd_squamate)
...
library(tidyr)
polygon_full_squamate_with_eco_nona<- st_as_sf(drop_na(polygon_data_full_squamate_with_eco))
...
plot_area <- plot_area +
  geom_sf(data = polygon_full_squamate_with_eco_nona, aes(fill = factor(mntdSigCal)))
