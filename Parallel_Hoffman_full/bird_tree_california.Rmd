...
b_tree<- read.tree("Bird_ML_dated001_trial2.tre") 
...
#MAKE SURE TO LOAD DPLYR PACKAGE OR THIS ALL GOES TO SHIT!
...
#removed this
{
  matched_species<-species_list%>% 
    filter(species_list$names%in%(master_phylogeny$tip.label))
  return(matched_species)
}


#cali_species%>% filter(cali_species$name%in%bird_tree_calib$tip.label)
...
print(species_tree$names)

cali_species_tree<- read.tree("cali_bird_tree.tre")
pd_app_picante(cali_species_tree, cali_species_tree)

plot(cali_species_tree, show.tip.label = FALSE )

write.tree( cali_species_tree, "cali_bird_tree.tre")
...
reserve_taxa<-remove_taxa(reserve_species, cali_species_tree)

#319 taxa observed in the UC reserves, 286 exist in the phylogeny

#286/319 (80.65%) 

reserve_species_tree<- sample_tree_generator(reserve_taxa,cali_bird_phylo)
...
ggtree(cali_species_tree, layout = "fan") +
    geom_tiplab(aes(subset = label%in% BX_taxa$names), size=1.2, colour = 'red')
ggsave("box_springs_reserve_poster.png", dpi = 1200)
...
ggtree(cali_species_tree, layout = "fan") +
    geom_tiplab(aes(subset = label%in% SGM_taxa$names), size=1.2, colour = 'red')
ggsave("Sweeney_Granite_Mountains.png", dpi = 1200)
...
ggtree(cali_species_tree, layout = "fan") +
    geom_tiplab(aes(subset = label%in% VC_taxa$names), size=1.2, colour = 'red')
ggsave("Valentine_Camp_Reserve.png", dpi = 1200)
...
tree_sizes= seq(10, 600, by=5) #this is a list of tree sizes. 
...
pd_low_fit<- drc::drm(pd ~ sizes, data = pd_low,fct = LL.4())
pd_med_fit<-drc::drm(pd ~ sizes, data = pd_med,fct = LL.4())
pd_high_fit<- drc::drm(pd ~ sizes, data = pd_high,fct = LL.4())
...
model_pd3 <- drc::drm(pd ~ sizes, data = pd_low,fct = LL.4())
summary(model_pd3)
model_pd4<- drc::drm (pd~sizes, data = pd_high, fct = LL.4())


plot(pd~sizes, data = pd_low)
lines(predict(model_pd3)~sizes, data = pd_low, type = "l", col = "blue")

summary(model_pd3)
plot(pd~sizes, data = pd_high)
lines(predict(model_pd4)~sizes, data = pd_high, type = "l", col = "blue")

plot(predict(model_pd3)~sizes, data = pd_low, type = "l", col = "blue")
lines(predict(model_pd4)~sizes, data = pd_high, type = "l", col = "blue")
points(bird_statistics$family_sizes, bird_statistics$pd_vals, cex = 0.5)
...
range_predict_pd<- function(sample_size, model_low, model_mean, model_high)
{
  lower<- model_low$coefficients[2] + ((model_low$coefficients[3]-model_low$coefficients[2])/(1+exp(model_low$coefficients[1]*(log(sample_size)-log(model_low$coefficients[4])))))
  
mean<- model_mean$coefficients[2] + ((model_mean$coefficients[3]-model_mean$coefficients[2])/(1+exp(model_mean$coefficients[1]*(log(sample_size)-log(model_mean$coefficients[4])))))
  
upper<- model_high$coefficients[2] + ((model_high$coefficients[3]-model_high$coefficients[2])/(1+exp(model_high$coefficients[1]*(log(sample_size)-log(model_high$coefficients[4])))))
 return(c(lower[[1]], mean[[1]], upper[[1]]))
}
...
cali_bird_phylo<-read.tree("cali_tree.tre")
...
pd_significant<- rep(NA, length(trees_list))
...
for(i in 1:length(trees_list))
{
  ...
  pd_significant[i]<- is_significant(range_predict_pd(family_sizes[i],pd_low_fit, pd_med_fit, pd_high_fit),pd_vals[i])
  }
}
...
bird_statistics$pd_significance<-pd_significant
rownames(bird_statistics)<-rnames
write.csv(bird_statistics, file = "bird_statistics.csv")
...
plot(x, ylow, type = "l", ylab = "mpd", ylim = c(50,130), xlab = "tree size")
lines(x, yhigh, type = "l", col = 2)
polygon(c(x, rev(x)), c(ylow, rev(yhigh)),
        col = "#99CCFF")

points(mpd_vals~family_sizes, data = bird_statistics[mpd_significant == TRUE,], pch = 16, cex = 1, col = 'red')
points(mpd_vals~family_sizes, data = bird_statistics[mpd_significant == FALSE,], pch = 16, cex = 1, col = 'black')
...
plot(x, ylow, type = "l", ylab = "mpd", ylim = c(0,160), xlab = "tree size")
lines(x, yhigh, type = "l", col = 2)
polygon(c(x, rev(x)), c(ylow, rev(yhigh)),
        col = "#99CCFF")
points(bird_statistics$family_sizes, bird_statistics$mntd_vals, cex = 0.5)

points(mntd_vals~family_sizes, data = bird_statistics[mntd_significant == TRUE,], pch = 16, cex = 1, col = 'red')
points(mntd_vals~family_sizes, data = bird_statistics[mntd_significant == FALSE,], pch = 16, cex = 1, col = 'black')


#for pd
x<- seq(0,250)
ylow<- pd_low_fit$coefficients[2] + ((pd_low_fit$coefficients[3]-pd_low_fit$coefficients[2])/(1+exp(pd_low_fit$coefficients[1]*(log(x)-log(pd_low_fit$coefficients[4])))))
yhigh<-pd_high_fit$coefficients[2] + ((pd_high_fit$coefficients[3]-pd_high_fit$coefficients[2])/(1+exp(pd_high_fit$coefficients[1]*(log(x)-log(pd_high_fit$coefficients[4])))))
plot(x, ylow, type = "l", ylab = "pd", ylim = c(0,3000), xlab = "tree size")
lines(x, yhigh, type = "l", col = 2)
polygon(c(x, rev(x)), c(ylow, rev(yhigh)),
        col = "#99CCFF")
points(bird_statistics$family_sizes, bird_statistics$pd_vals, cex = 0.5)

points(pd_vals~family_sizes, data = bird_statistics[pd_significant == TRUE,], pch = 16, cex = 1, col = 'red')
points(pd_vals~family_sizes, data = bird_statistics[pd_significant == FALSE,], pch = 16, cex = 1, col = 'black')
...
plot(pd_low$pd~ pd_low$sizes, type = "l", xlim = c(0,650), ylab = "pd", xlab = "tree size")
lines(pd_high$pd ~pd_high$sizes, type = "l")
polygon(c(pd_low$sizes, rev(pd_low$sizes)), c(pd_low$pd, rev(pd_high$pd)),
        col = "#6BD7AF")
points(bird_statistics$pd_vals~ bird_statistics$family_sizes, cex = 0.4)



#here is mntd raw
plot(mntd_high$mntd~ mntd_high$sizes, type = "l", ylim = c(0,100),xlab = "tree sizes", ylab = "mntd")
lines(mntd_low$mntd ~mntd_low$sizes, type = "l")
polygon(c(mntd_high$sizes, rev(mntd_high$sizes)), c(mntd_low$mntd, rev(mntd_high$mntd)),
        col = "#6BD7AF")
points(bird_statistics$mntd_vals~ bird_statistics$family_sizes, cex = 0.4)
x



### creating a dataframe that has only significantly low pd, mpd and mntd
sig_reserves<-bird_statistics[pd_significant == TRUE && mntd_significant == TRUE &&mpd_significant == TRUE,]


any_sig_reserve<- bird_statistics[pd_significant == TRUE | mntd_significant == TRUE | mpd_significant == TRUE,]

write.csv(any_sig_reserve, file = "reserves_with_at_least_one_low_value.csv")

ro###this is for finding some statistics about percent significant reserves. 

total_reserves<- count(bird_statistics)
sig_pd<- count(bird_statistics[pd_significant==TRUE,])
sig_mpd<-count(bird_statistics[mpd_significant==TRUE,])
sig_mntd<- count(bird_statistics[mntd_significant==TRUE,])

p_pd<- sig_pd/total_reserves
p_mpd<- sig_mpd/total_reserves
p_mntd<-sig_mntd/total_reserves


### for the entire reserve system

reserve_taxa

pd_reserve<- pd_app_picante(reserve_species_tree, cali_bird_phylo)
mpd_reserve<-mpd_app_picante(reserve_species_tree, cophen_touse)
mntd_reserve<-mntd_app_picante(reserve_species_tree, cophen_touse)

reserve_pd_significant<-is_significant(range_predict_pd(length(reserve_species_tree$tip.label), pd_low_fit, pd_med_fit, pd_high_fit), pd_reserve$PD)

mntd_reserve_significant<- is_significant(range_pred(length(reserve_species_tree$tip.label), mntd_low_fit, mntd_med_fit, mntd_high_fit), mntd_reserve)

reserve_mpd_significant<- is_significant(range_pred(length(reserve_species_tree$tip.label),model_low, model_med, model_high),mpd_reserve)
...
#this part is for trying to build the california bird phylogeny with specific bird groups. Essentially want to make this bird tree pretyty. 

#step 1: find the nodes of interest 


calibration_nodes<- c(Calib1_node, Calib2_node, Calib3_node, Calib4_node, Calib6_node, Calib7_node, Calib8_node, Calib9_node, Calib10_node, Calib11_node, Calib12_node, Calib13_node, Jarvis2_node, Jarvis4_node, Jarvis5_node, Jarvis7_node, Jarvis9_node, Jarvis12_node, Jarvis21_node, Jarvis21_node, Jarvis22_node, Jarvis23_node, Jarvis29_node, Jarvis31_node, Jarvis35_node, Jarvis38_node) 


#look at interesting nodes of california tree


ggtree(cali_bird_phylo,layout = "circular")+ geom_text(aes(label=node), hjust=-.3, cex = 3)


#try some highlighting

ggtree(cali_bird_phylo, layout="circular") + geom_hilight(node=967, fill="steelblue", alpha=0.3) + geom_hilight(node=1129, fill="darkgreen", alpha=0.3) + geom_highlight(node = 1208, fill = "orange", alpha = 0.3 )
ggsave("clades_of_interest.png", dpi = 1200)



#trying to do some further pruning

cali_tips<-cali_bird_phylo$tip.label
genera<-unique(sapply(strsplit(cali_tips,"_"),function(x) x[1]))
ii<-sapply(genera,function(x,y) grep(x,y)[1],y=cali_tips)
cali_tree_trimmed<-drop.tip(cali_bird_phylo,setdiff(cali_bird_phylo$tip.label,cali_tips[ii]))
ggtree(cali_tree_trimmed, layout = "circular")

ggtree(cali_tree_trimmed,layout = "circular", cex = 0.1) + geom_tiplab(size = 2)
